...
export default function EditProfile({ activeTab, translatedTexts }) {
  ...
  const adminCancelSubscription = async () => {
    ...
    if (!userData?.subscriptionId) {
      setAlertMessage({
        type: "danger",
        content:
          translatedTexts?.adminNoActiveSubscriptionText ||
          "Aucun abonnement actif trouvé pour cet utilisateur.",
        showAlert: true,
      });
      return;
    }
    if (userData?.subscriptionStatus === "lifetime" || userData?.lifetimeAccess) {
      setAlertMessage({
        type: "danger",
        content:
          translatedTexts?.adminLifetimeSubscriptionExistsText ||
          "Cet utilisateur a un abonnement à vie.",
        showAlert: true,
      });
      return;
    }
    try {
      ...
      if (!res.ok || !data?.success) {
        throw new Error(
          data?.error ||
            translatedTexts?.failedToCancelSubscriptionText ||
      ...
      setAlertMessage({
        type: "success",
        content:
          translatedTexts?.adminCancelSubscriptionSuccessText ||
  ...
  const adminSetLifetimeOffer = async (enabled) => {
    ...
    try {
      ...
      if (!res.ok || !data?.success) {
        throw new Error(
          data?.error ||
            translatedTexts?.failedToUpdateLifetimeOfferText ||
            "Failed to update lifetime offer"
        );
      }
  ...
  const saveUserEdits = async () => {
    ...
    try {
      ...
      if (!res.ok || !data?.success) {
        throw new Error(
          data?.error ||
            translatedTexts?.failedToSaveUserText ||
            "Failed to save user"
        );
      }
  ...
  if (loading) {
    return <p>{translatedTexts?.loadingText || "Loading..."}</p>;
  }

  // Dacă nu există date despre utilizator, afișăm un mesaj de eroare
  if (!userData) {
    return (
      <p>{translatedTexts?.noUserDataFoundText || "No user data found."}</p>
    );
  }
      ...
      <div className="row pb-50 mb-10">
        <div className="col-auto" style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {userData?.deletedAccount?.isDeleted ? (
            ...
            >
              {translatedTexts?.deletedBadgeText || "Compte supprimé"}
            </span>
          ) : null}
          {/* <div className="mt-10">
        ...
        <form className="contact-form row y-gap-30">
          ...
          <div className="col-md-6">
            ...
            <input
              ...
              type="text"
              placeholder={
                translatedTexts?.usernamePlaceholderText || "Nume Utilizator"
              }
          ...
          <div className="col-md-6">
            <label className="text-16 lh-1 fw-500 text-dark-1 mb-10">
              {translatedTexts?.ageLabelText || "Age"}
            </label>
            <input
              readOnly={!isEditMode}
              required
          ...
          <div className="col-md-6">
            ...
            <input
              ...
              type="text"
              placeholder={
                translatedTexts?.emailPlaceholderText || "Email"
              }
          ...
          <div className="col-12">
            ...
            <textarea
              ...
              required
              placeholder={
                translatedTexts?.aboutMePlaceholderText || "Despre mine"
              }
              rows="7"
          ...
          <div className="col-12">
            ...
            <textarea
              ...
              required
              placeholder={
                translatedTexts?.addressPlaceholderText || "Despre mine"
              }
              rows="7"
          ...
          {userData.responses ? (
            <div className="col-12">
              ...
              >
                {({ loading }) =>
                  loading
                    ? translatedTexts?.generatingPdfText || "Generare PDF..."
                    : translatedTexts?.downloadAnswersPdfText ||
                      "Descarcă PDF cu răspunsuri"
                }
              </PDFDownloadLink>
            </div>
          ) : (
            ...
            >
              ...
              >
                {translatedTexts?.noQuizCompletedText ||
                  ...
                  <div className="col-12 mt-20">
                    {isCancelingSubscription ? (
                      ...
                      >
                        ...
                        <span style={{ fontWeight: "bold" }}>
                          {translatedTexts?.cancelSubscriptionLoadingText ||
                            "Annulation en cours..."}
                        </span>
                      </div>
                    ) : (
      ...
      {showCancelSubscriptionDialog && (
        ...
        >
          <p style={{ fontWeight: "bold" }}>
            {translatedTexts?.cancelSubscriptionDialogTitleText ||
              "Confirmer l'annulation de l'abonnement pour cet utilisateur ?"}
          </p>
          <p style={{ marginTop: 8 }}>
            {translatedTexts?.cancelSubscriptionDialogDescriptionText ||
              "Cela désactivera le renouvellement automatique (Stripe: cancel_at_period_end)."}
          </p>
          <div style={{ display: "flex", gap: "10px", marginTop: "15px" }}>
            <button
              className="button -md -red-1 text-white"
            ...
            >
              {translatedTexts?.cancelSubscriptionDialogConfirmText ||
                "Annuler l'abonnement"}
            </button>
            <button
              className="button -md -gray-1 text-dark-1"
