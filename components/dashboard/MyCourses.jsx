...
export default function MyCourses({ translatedTexts }) {
  ...
  const handleExportUsers = async (format) => {
    try {
      ...
      if (!token) {
        alert(translatedTexts?.authRequiredText || "Not authenticated");
        return;
      }
      ...
      if (!res.ok) {
        ...
        throw new Error(
          err?.error ||
            translatedTexts?.failedToExportUsersText ||
      ...
      alert(
        error?.message || translatedTexts?.exportFailedText || "Export failed"
      );
    } finally {
      setExportingFormat("");
    }
      ...
      <div className="dashboard__content bg-light-4">
        <div className="row pb-50 mb-10">
          ...
          <div className="col-auto" style={{ display: "flex", alignItems: "center" }}>
            ...
            >
              {exportingFormat === "csv"
                ? translatedTexts?.exportingText || "Export..."
                : translatedTexts?.exportCsvText || "Export CSV"}
            </button>
            <button
              type="button"
              className="button -sm -purple-1 text-white"
            ...
            >
              {exportingFormat === "xlsx"
                ? translatedTexts?.exportingText || "Export..."
                : translatedTexts?.exportExcelText || "Export Excel"}
            </button>
          </div>
