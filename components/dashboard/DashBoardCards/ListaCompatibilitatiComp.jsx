...
export default function ListCompatibilitati({
  ...
  return (
    <tr>
      <td>{data.username}</td>
      <td>{data.gender ? data.gender : translatedTexts?.naText || "N/A"}</td>
      <td>
        {data.isActivated
          ? translatedTexts?.compatibilityActiveAccountText || "Cont activ"
          : translatedTexts?.compatibilityInactiveAccountText ||
            "Cont inactiv"}
      </td>
      <td>{compatibility}%</td>
      <td>
      ...
      <td style={{ display: "flex", gap: "10px" }}>
        ...
        >
          {({ loading }) => (
            ...
            >
              {loading
                ? translatedTexts?.compatibilityGeneratingPdfText ||
                  "Génération du PDF..."
                : translatedTexts?.compatibilityDownloadPdfText ||
                  "Télécharger le PDF"}
            </button>
          )}
        </PDFDownloadLink>
        ...
        >
          {isCompatible
            ? translatedTexts?.compatibilityRemoveText ||
              "Supprimer la compatibilité"
            : translatedTexts?.compatibilityMarkText ||
